...
int main() {
  ...
  print_banner();
  printf("%s",
         FORMAT_STRING_TO_ANSI(ANSI_GREEN, ANSI_BOLD,
                               "Parsing Quiz data from the json file...\n"));
  ...
  sprintf(buffer, "%sParsed %s%d%s%s questions from data.json%s\n", ANSI_GREEN,
          ANSI_BOLD, res.count, ANSI_RESET, ANSI_GREEN, ANSI_RESET);
  printf("%s\n", buffer);
  int    score = 0, q_size, t_top = -1;
  double total_time;
  printf("%s", FORMAT_STRING_TO_ANSI(ANSI_BLUE, ANSI_BOLD,
  ...
  printf("%s", FORMAT_STRING_TO_ANSI(
                   ANSI_CYAN, ANSI_BOLD,
  ...
  for (int ques = 0; ques < q_size; ques++) {
    ...
    printf("%s%sQ%d: %s%s\n", ANSI_GREEN, ANSI_BOLD, ques + 1, curr.question,
           ANSI_RESET);
    ...
    printf("%s", FORMAT_STRING_TO_ANSI(ANSI_MAGENTA, ANSI_BOLD,
                                       "Your choice (a, b, c, d): "));
  ...
  for (int i = 0; i < q_size; i++) {
    printf("%s%sQ%i: %s%s\n", ANSI_BLUE, ANSI_BOLD, i + 1,
           questions[i].question, ANSI_RESET);
    ...
    printf("%s%sCorrect: (%c) %s%s\n", ANSI_GREEN, ANSI_BOLD,
           _1234_to_abcd(q.answer), correct, ANSI_RESET);
    printf("%s%s[%.2f secs] Your Answer: (%c) %s%s\n",
           abcd_to_1234(responses[i]) == q.answer ? ANSI_GREEN : ANSI_RED,
           ANSI_BOLD, res_time[i], responses[i], response, ANSI_RESET);
    if (abcd_to_1234(responses[i]) == q.answer)
      score += 1;
    total_time += res_time[i];
  }
  printf("%s%sScore: %i/%i\nTotal Time: %.2f\nAverage Time: %.2f%s\n",
         ANSI_CYAN, ANSI_BOLD, score, q_size, total_time, total_time / q_size,
         ANSI_RESET);
}
...
char _1234_to_abcd(int a) {
  if (a >= 1 && a <= 4) {       // Ensure a is within valid range
    return (char)(a + 'a' - 1); // Convert to lowercase letters
  }
  return '?'; // Return '?' for invalid input
}

char* s_create(int size) { return malloc(size * sizeof(char)); }

int s_is_empty(int top) { return top == -1; }

int s_is_full(int size, int top) { return size == top; }
